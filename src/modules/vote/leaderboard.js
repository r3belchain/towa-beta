    ...
    async () => {
      ...
      try {
        ...
        if (channel && embedFinal) {
          await channel.send({
            content:
              "📢 **REKAP LEADERBOARD VOTE** 📢\n\nTerima kasih untuk warga yang sudah berpartisipasi di bulan ini! **ROLE VOTE** akan dipasang oleh Mekanik TOWA secara manual.\n\n*Poin leaderboard resmi telah di-reset ke 0 untuk menyambut bulan baru.*",
            embeds: [embedFinal],
          });
          console.log(
            "📢 [CRON] Rekap leaderboard berhasil dikirim ke channel.",
          );
        }
      } catch (discordErr) {
        console.error(
